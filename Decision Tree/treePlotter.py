...
def plot_node(feature_name, num_sample, center_pt, parent_pt, node_type, ax_):
    node_text = feature_name + ': ' + str(num_sample)
...
def plot_tree(my_tree, parent_pt, node_text, ax_):
    ...
    if total_high == 0:  # total_high为零时，表示就直接为一个叶节点。因为西瓜数据集的原因，在预剪枝的时候，有时候会遇到这种情况。
        plot_node(my_tree.leaf_class, my_tree.num_sample, center_pt, parent_pt, leaf_node, ax_)
        return

    plot_node(my_tree.feature_name, my_tree.num_sample, center_pt, parent_pt, decision_node, ax_)
    y_off -= 1 / total_high

    for key in my_tree.subtree.keys():
        if my_tree.subtree[key].is_leaf:
            x_off += 1 / total_num_leaf
            plot_node(str(my_tree.subtree[key].leaf_class), my_tree.subtree[key].num_sample, (x_off, y_off), center_pt, leaf_node, ax_)
